...
{
    public partial class ActualizarAuditoriaPop : System.Web.UI.Page
    {
        string fecha_actual;
        ...
        {
            bool exitoIdSistemaQA = true;
            if (!ddlDepartamento.SelectedValue.Equals("0")) exitoIdSistemaQA = (obeAuditoriaTipo.Departamento_id.ToString().Equals(ddlDepartamento.SelectedValue));
            return (exitoIdSistemaQA);
        }
        ...
        {
            TB_PlanAccionBL _TB_PlanAccionBL = new TB_PlanAccionBL();
            if (_AUD_AuditoriaBL.EliminarAUD_Auditoria(Convert.ToInt32(Request.QueryString["Auditoria_id"])))
                ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "jAlert", "jAlert('Registro eliminado');", true);
            else
                ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "jAlert", "jAlert('No se pudo eliminar el registro');", true);
